      ...
      <div className="flex-grow-1 overflow-auto">
        <div className="d-flex flex-column align-items-start justify-content-end px-3">
          {selectedConversion.messages.map((message, index) => {
            const lastMessage = selectedConversion.messages.length - 1 === index;
            return (
              <div ref={lastMessage ? setRef : null} key={index} className={`${message.fromMe ? 'align-self-end': ''}my-1 d-flex flex-column`}>
                <div className={`${message.fromMe ? 'bg-primary text-white' : 'border'} rounded px-2 py-1`}>{message.text}</div>
